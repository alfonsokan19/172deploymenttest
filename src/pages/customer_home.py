                            ...
                            [
                                ...
                                html.Div(
                                    dbc.Form(
                                        dbc.Row(
                                            [
                                                dbc.Label("Search Customers", width=2),
                                                dbc.Col(
                                                    dbc.Input(
                                                        type='text',
                                                        id='customer_namefilter',
...
# @app.callback(
#     [
#         Output(component_id='customerhome_customerlist', component_property='children')
#     ],
#     [
#         Input(component_id='url', component_property='pathname'),
#         Input(component_id='customer_namefilter', component_property='value'),  # changing the text box value should update the table
#     ]
# )
# def customer_loadcustomerlist(pathname, searchterm):
#     if pathname == '/information/customer':
#         # 1. Obtain records from the DB via SQL
#         # 2. Create the html element to return to the Div
#         sql = """ SELECT concat(customer_fn, ' ', customer_mn, ' ', customer_ln), customer_phone_number, customer_address, customer_id
#                   FROM customer
#                   WHERE NOT customer_delete_ind
#               """
#         values = []  # blank since I do not have placeholders in my SQL
#         cols = ['Customer name', 'Phone number', 'Address', 'ID']
#         ### ADD THIS IF BLOCK
#         if searchterm:
#             # We use the operator ILIKE for pattern-matching
#             sql += " AND concat(customer_fn, ' ', customer_mn, ' ', customer_ln) ILIKE %s"
...
#                     html.Div(
#                         dbc.Button('Edit',
#                                 href=f'customer/customer_profile?mode=edit&id={customer_id}',
...
#             df = df[['Customer name', 'Phone number', "Address", "Action"]]
